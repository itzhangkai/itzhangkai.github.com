...
carol.timeScroll = null;  //挂载整屏切换动画的实例

carol.currentStep = "step1";

carol.init = function(){

	carol.resize(); //设置每一屏的高度和top值

	carol.events(); //配置事件

	carol.configIntAnimate();//配置导航条的动画

	carol.button3D(".start",".state1",".state2",0.3);
	carol.button3D(".button1",".state1",".state2",0.3);
	carol.button3D(".button2",".state1",".state2",0.3);

	//设置每一屏中img的百分比

	carol.imgWidth($(".scene img"));

	carol.configTimeScroll(); //配置横屏切换的动画
...
$(document).ready( carol.init );

//配置事件
carol.events = function(){
	

	carol.nav(); //执行导航条的鼠标移入移除的动画
	...
	$(window).bind("scroll",carol.scrollStatus);
	...
	$(window).bind("mouseup",carol.mouseupFn)
	...
	function mousewheelFn(ev,direction){
		...
		if( direction<1 ){  //向下滚动
			//console.log("next");
			carol.changeStep("next");
		}else{  //向上滚动
			console.log("prev");
			carol.changeStep("prev");
		};
		clearTimeout(timer);
	...
	$(window).resize( carol.resize );


};
//当mouseup的时候，让当前这一屏到达某个状态
carol.mouseupFn = function(){
	//在滚动过程中计算出一个比例
	var scale = carol.scale();
	//得到当前页面到达的某个时间点
	var times = scale * carol.timeScroll.totalDuration();

	//获取到上一个状态和下一个状态
	var prevStep = carol.timeScroll.getLabelBefore(times);
	var nextStep = carol.timeScroll.getLabelAfter(times);

	//获取到上一个状态的时间和下一个状态的时间
	var prevTime = carol.timeScroll.getLabelTime(prevStep);
	var nextTime = carol.timeScroll.getLabelTime(nextStep);
	...
	*/
	...
	carol.timeScroll.tweenTo(step);
	//-----------------当松开鼠标时候，控制滚动条到达某个状态计算出的距离-----------------------
	//获取动画的总时长
	var totalTime = carol.timeScroll.totalDuration();
	//获取到要到达的状态的时间
	var afterTime = carol.timeScroll.getLabelTime(step);
	//获取到滚动条能够滚动的最大的高度
	var maxH = $("body").height() - $(window).height();
	//计算出滚动条滚动的距离
	var positionY = afterTime/totalTime * maxH;
	//滚动条滚动的距离的持续时间
	var d = Math.abs( carol.timeScroll.time() - afterTime );

	var scrollAnimate = new TimelineMax();

	scrollAnimate.to("html,body",d,{scrollTop:positionY});

	carol.currentStep = step;
}

//计算滚动条在滚动过程中的一个比例

carol.scale = function(){
...
//在滚动条滚动的过程中，计算页面中应该到哪一个时间点上去
carol.scrollStatus = function (){
	var times = carol.scale() * carol.timeScroll.totalDuration();
	//当滚动条在滚动的过程中，让页面中的动画到打某个时间点
	carol.timeScroll.seek(times,false);
}

//切换整屏并且计算滚动条的距离

carol.changeStep = function(value){
	if( value === "next" ){ //向下切换

		//获取当前的时间
		var currentTime = carol.timeScroll.getLabelTime(carol.currentStep);

		//获取到下一个状态的字符串
		var afterStep = carol.timeScroll.getLabelAfter(currentTime);
		...
		var totalTime = carol.timeScroll.totalDuration();
		//获取到下一个状态的时间
		var afterTime = carol.timeScroll.getLabelTime(afterStep);
		//获取到滚动条能够滚动的最大的高度
		var maxH = $("body").height() - $(window).height();
		...
		//carol.timeScroll.tweenTo(afterStep);
		//记录当前的状态为下一个状态，方便继续切换到下一个状态上
		carol.currentStep = afterStep;
		...
		//获取到上一个状态的字符串
		var beforeStep = carol.timeScroll.getLabelBefore(currentTime);
		...
		var totalTime = carol.timeScroll.totalDuration();
		//获取到下一个状态的时间
		var BeforeTime = carol.timeScroll.getLabelTime(beforeStep);
		//获取到滚动条能够滚动的最大的高度
		var maxH = $("body").height() - $(window).height();
		...
		//滚动条滚动的距离的持续时间
		var d = Math.abs( carol.timeScroll.time() - BeforeTime );
		...
		//carol.timeScroll.tweenTo(beforeStep);
		//记录当前的状态为上一个状态，方便继续切换到上一个状态上
		carol.currentStep = beforeStep;
	}
}

//配置整屏切换的动画以及每一屏中的小动画

carol.configTimeScroll = function(){

	var time = carol.timeScroll ? carol.timeScroll.time() : 0;

	if( carol.timeScroll ) carol.timeScroll.clear();

	carol.timeScroll = new TimelineMax();

		// 当从第二屏切换到第一屏的时候，让第二屏里面的动画时间点重归0
		carol.timeScroll.to(".scene1",0,{onReverseComplete:function(){
			twoAnimate.timeline.seek(0,false);
		}},0);

		carol.timeScroll.to(".footer",0,{top:"100%"});

		carol.timeScroll.add("step1");
	carol.timeScroll.to(".scene2",0.8,{top:0,ease:Cubic.easeInOut});
	carol.timeScroll.to({},0.1,{onComplete:function(){
		menu.changeMenu("menu_state2");  //切换到第二屏调用的函数，同时传入导航条背景颜色变化的class名字
	},onReverseComplete:function(){
		menu.changeMenu("menu_state1"); 
	}},"-=0.2");
	//当切换到第二屏上的时候，翻转第二屏上的第一个动画
	carol.timeScroll.to({},0,{onComplete:function(){
		twoAnimate.timeline.tweenTo("state1");
	}},"-=0.2");

		carol.timeScroll.add("step2");
	...
	carol.timeScroll.to({},0.4,{});

		carol.timeScroll.add("point1");
	...
	carol.timeScroll.to({},0.4,{});
		...
		carol.timeScroll.add("point3");

	// --- 主动画中配置第二萍的小动画 end


	carol.timeScroll.to(".scene3",0.8,{top:0,ease:Cubic.easeInOut,onReverseComplete:function(){
	...
	carol.timeScroll.to({},0.4,{});
		...
		carol.timeScroll.add("threeSate");

	// --- 主动画中配置第三屏的小动画 end



	carol.timeScroll.to(".scene4",0.8,{top:0,ease:Cubic.easeInOut});
		carol.timeScroll.add("step4");

	//滚动到第五屏的时候，要让第四屏滚出屏幕外
	carol.timeScroll.to(".scene4",0.8,{top:-$(window).height(),ease:Cubic.easeInOut});
	//当可视区域大于950，那么就让导航条隐藏起来
	if($(window).width()>950){
		carol.timeScroll.to(".menu_wrapper",0.8,{top:-110,ease:Cubic.easeInOut},"-=0.8");
	}else{
		$(".menu_wrapper").css("top",0);
	}

	carol.timeScroll.to(".scene5",0.8,{top:0,ease:Cubic.easeInOut,onReverseComplete:function(){
		fiveAnimate.timeline.seek(0,false);
	}},"-=0.8");
	carol.timeScroll.to({},0.1,{onComplete:function(){
		fiveAnimate.timeline.tweenTo("fiveState");
	}},"-=0.2");
		carol.timeScroll.add("step5");

	carol.timeScroll.to(".scene5",0.5,{top:-$(".footer").height(),ease:Cubic.easeInOut});
	carol.timeScroll.to(".footer",0.5,{top:$(window).height()-$(".footer").height(),ease:Cubic.easeInOut},"-=0.5");

		carol.timeScroll.add("footer");

	carol.timeScroll.stop();
	//当改变浏览器的大小时，让动画走到之前已经到达的时间点
	carol.timeScroll.seek(time);
}

//配置导航条的动画
carol.configIntAnimate = function(){
	...
	initAnimate.staggerTo( ".scene1_1 .rotatediv",2,{opacity:1,rotationX:0,ease:Elastic.easeOut},1 );
...
// 3D翻转效果
carol.button3D = function(obj,element1,element2,d){
...
carol.imgWidth = function(elementImg){
...
twoAnimate.init = function(){
	...
	twoAnimate.timeline.to(".scene2_1 .scene2_div1",1,{opacity:1,rotationX:0,ease:Elastic.easeOut} );
	...
	twoAnimate.timeline.to(".scene2_1 .scene2_div1",0.2,{opacity:0,rotationX:90} );
	...
	twoAnimate.timeline.to(".scene2_2 .scene2_div2",1,{opacity:1,rotationX:0,ease:Elastic.easeOut} );
	...
	twoAnimate.timeline.to(".scene2_2 .scene2_div2",0.3,{opacity:0,rotationX:90,ease:Cubic.easeInOut});
	...
	twoAnimate.timeline.to(".scene2_3 .scene2_div3",1,{opacity:1,rotationX:0,ease:Elastic.easeOut});
	//第三个按钮
	...
	twoAnimate.timeline.to(".scene2_3 .scene2_div3",0.3,{opacity:0,rotationX:90,ease:Cubic.easeInOut});
	...
	twoAnimate.timeline.to(".scene2_4 .scene2_div4",1,{opacity:1,rotationX:0,ease:Elastic.easeOut});
	//第三个按钮
...
threeAnimate.init = function(){
	...
	threeAnimate.timeline.to( ".scene3 .scene3_div1",0.2,{opacity:1,rotationX:0,ease:Elastic.easeOut});
	threeAnimate.timeline.add("threeSate1");

	threeAnimate.timeline.to(".step3_1 img",0.3,{opacity:0,rotationX:-90,ease:Cubic.easeInOut});
	threeAnimate.timeline.to( ".scene3 .scene3_div1",0.4,{opacity:0,rotationX:-90,ease:Cubic.easeInOut});
	threeAnimate.timeline.to(".step3_2 img",0.3,{opacity:1,rotationX:0,ease:Cubic.easeInOut});
	threeAnimate.timeline.to(".scene3 .scene3_div2",0.3,{opacity:1,rotationX:0,ease:Cubic.easeInOut});
